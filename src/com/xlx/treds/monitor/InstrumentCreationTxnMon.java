...
public class InstrumentCreationTxnMon extends DefaultHandler {
    public InstrumentCreationTxnMon(){
    	super();
    }
    ...
    protected List getBeanList(Connection pConnection, Map<String, Object> pFilterMap, int pRecordCount, IAppUserBean pAppUserBean) throws Exception {
        ...
        HashMap<String, InstrumentBean> lHash = new HashMap<>();
        String lKey = null;
        InstrumentBean lPreviousInstBean = null;
        List lRtnList = new ArrayList<Object>();
        Set<Long> lKeySet = new HashSet<Long>();
        for (InstrumentBean lInstBean :(List<InstrumentBean>) lList) {
        	lKey = lInstBean.getPurchaser()+CommonConstants.KEY_SEPARATOR+lInstBean.getSupplier()+CommonConstants.KEY_SEPARATOR+lInstBean.getAmount();
        	if (!lHash.containsKey(lKey)) {
        		lHash.put(lKey, lInstBean);
        	}else {
        		lPreviousInstBean = lHash.get(lKey);
        		Long lDateDiff = OtherResourceCache.getInstance().getDiffInDays(lPreviousInstBean.getRecordCreateTime(), lInstBean.getRecordCreateTime());
        		if (lDateDiff <= new Long(7) && lDateDiff >= new Long (0) ) {
        			if (!lKeySet.contains(lInstBean.getId())) {
        				lKeySet.add(lInstBean.getId());
        				lRtnList.add(lInstBean);
        			}
        			if (!lKeySet.contains(lPreviousInstBean.getId())) {
        				lKeySet.add(lPreviousInstBean.getId());
        				lRtnList.add(lPreviousInstBean);
        			}
        		}
        		lHash.put(lKey, lInstBean);
        	}
