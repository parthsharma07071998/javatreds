...
@Singleton
@Path("/instchk")
public class InstrumentCheckerResource {
    private static final Logger logger = LoggerFactory.getLogger(InstrumentCheckerResource.class);

    private InstrumentBO instrumentBO;
    private BeanMeta instrumentBeanMeta;
	private List<String> defaultListFields, lovFields;
	
    public InstrumentCheckerResource() {
        super();
    ...
    public void pageChecker(@Context HttpServletRequest pRequest, @Context HttpServletResponse pResponse) throws Exception {
        pRequest.getRequestDispatcher("/WEB-INF/instchk.jsp").forward(pRequest, pResponse);
    }
    
    @GET
    @Produces(MediaType.APPLICATION_JSON)
    @Secured(secKey="instchk-view")
    @Path("/{id}")
    ...
    public String list(@Context ExecutionContext pExecutionContext, @Context HttpServletRequest pRequest, 
        ...
        List<InstrumentBean> lInstrumentList = instrumentBO.findListChecker(pExecutionContext, lFilterBean, null, lUserBean , lFilterBean.getFiltHistFlag()==CommonAppConstants.YesNo.Yes?Boolean.TRUE:Boolean.FALSE, -1);
        List<Object[]> lResults = new ArrayList<Object[]>();
        for (InstrumentBean lInstrumentBean : lInstrumentList) {
            instrumentBO.setTabForChecker(lInstrumentBean, lFilterBean.getFiltHistFlag()==CommonAppConstants.YesNo.Yes?Boolean.TRUE:Boolean.FALSE);
    ...
    public Object listDownload(@Context ExecutionContext pExecutionContext, @Context HttpServletRequest pRequest,
        ...
        List<String> lFields = (List<String>)lMap.get(BeanMeta.PARAM_COLUMNNAMES);
        if (lFields == null) lFields = defaultListFields;
        List<InstrumentBean> lInstrumentList = instrumentBO.findListChecker(pExecutionContext, lFilterBean, null, lUserBean, lFilterBean.getFiltHistFlag()==CommonAppConstants.YesNo.Yes?Boolean.TRUE:Boolean.FALSE, AppConstants.RECORDS_DOWNLOAD_ALL);
        ...
        String lFileName = "InstrumentsChecking.csv";
    ...
    public String updateStatusApprove(@Context ExecutionContext pExecutionContext, @Context HttpServletRequest pRequest, 
        String pFilter) throws Exception {
    	return updateStatus(pExecutionContext, pRequest, pFilter, InstrumentBean.Status.Checker_Approved);
    }
    
    @POST
    @Consumes(MediaType.APPLICATION_JSON)
    @Secured(secKey="instchk-reject")
    @Path("/status/chkrej")
    public String updateStatusReject(@Context ExecutionContext pExecutionContext, @Context HttpServletRequest pRequest, 
        String pFilter) throws Exception {
    	return updateStatus(pExecutionContext, pRequest, pFilter, InstrumentBean.Status.Checker_Rejected);
    }
    
    @POST
    @Consumes(MediaType.APPLICATION_JSON)
    @Secured(secKey="instchk-return")
    @Path("/status/chkret")
    public String updateStatusReturn(@Context ExecutionContext pExecutionContext, @Context HttpServletRequest pRequest, 
        String pFilter) throws Exception {
        return updateStatus(pExecutionContext, pRequest, pFilter, InstrumentBean.Status.Checker_Returned);
    }
