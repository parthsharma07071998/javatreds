...
public class NPCIPaymentSettlor implements IPaymentSettlor {
    private static final Logger logger = LoggerFactory.getLogger(NPCIPaymentSettlor.class);
	//
	...
	private List<Long> modifiedReadyObliList = null;
	//
	//Helper variables
	//temp entity list for determining type
	private HashMap<String, NPCIPaymentSettlor.EntityType> appEntitiesType = null;	//Key=EntityCode/Domain
    ...
    private String[] modifiedReadyObliListStr = null;
    ...
    //
	//FU STATUS UPDATE
	private static int IDX_RESULT_SIZE = 7;
	//
	//FU STATUS UPDATE
	...
	private static String[] resultText = new String[] {"Leg1 Success", "Leg1 Fail", "Leg1 Fail", "Leg2 Success", "Leg2 Fail", 	"Leg3 Success", "Leg3 Fail"};
	private static String[] resultOldFUStatus = new String[] {FactoringUnitBean.Status.Factored.getCode(), FactoringUnitBean.Status.Factored.getCode(), FactoringUnitBean.Status.Factored.getCode(), FactoringUnitBean.Status.Leg_1_Settled.getCode(), FactoringUnitBean.Status.Leg_1_Settled.getCode() ,  FactoringUnitBean.Status.Leg_3_Generated.getCode(),  FactoringUnitBean.Status.Leg_3_Generated.getCode() };
	private static String[] resultNewFUStatus = new String[] {FactoringUnitBean.Status.Leg_1_Settled.getCode(), FactoringUnitBean.Status.Leg_1_Failed.getCode(), FactoringUnitBean.Status.Leg_1_Failed.getCode(), FactoringUnitBean.Status.Leg_2_Settled.getCode(), FactoringUnitBean.Status.Leg_2_Failed.getCode() , FactoringUnitBean.Status.Leg_3_Settled.getCode(),  FactoringUnitBean.Status.Leg_3_Failed.getCode() };
	private static String[] resultObliType = new String[] {ObligationBean.Type.Leg_1.getCode(), ObligationBean.Type.Leg_1.getCode(), ObligationBean.Type.Leg_1.getCode(), ObligationBean.Type.Leg_2.getCode(), ObligationBean.Type.Leg_2.getCode(),  ObligationBean.Type.Leg_3.getCode(), ObligationBean.Type.Leg_3.getCode()};
	private static String[] resultObliTxnType = new String[] {ObligationBean.TxnType.Credit.getCode(), ObligationBean.TxnType.Credit.getCode(), ObligationBean.TxnType.Debit.getCode(), ObligationBean.TxnType.Debit.getCode(), ObligationBean.TxnType.Debit.getCode(), ObligationBean.TxnType.Debit.getCode(), ObligationBean.TxnType.Debit.getCode()};
	private static String[] resultObliStatus = new String[] {ObligationBean.Status.Success.getCode(), ObligationBean.Status.Failed.getCode(), ObligationBean.Status.Failed.getCode(), ObligationBean.Status.Success.getCode(), ObligationBean.Status.Failed.getCode(), ObligationBean.Status.Success.getCode(), ObligationBean.Status.Failed.getCode()};
	//
	private static String[] resultOldInstStatus = new String[] {InstrumentBean.Status.Factored.getCode(), InstrumentBean.Status.Factored.getCode(), InstrumentBean.Status.Factored.getCode(), InstrumentBean.Status.Leg_1_Settled.getCode(), InstrumentBean.Status.Leg_1_Settled.getCode(), InstrumentBean.Status.Leg_3_Generated.getCode(), InstrumentBean.Status.Leg_3_Generated.getCode() };
	private static String[] resultNewInstStatus = new String[] {InstrumentBean.Status.Leg_1_Settled.getCode(), InstrumentBean.Status.Leg_1_Failed.getCode(), InstrumentBean.Status.Leg_1_Failed.getCode(), InstrumentBean.Status.Leg_2_Settled.getCode(), InstrumentBean.Status.Leg_2_Failed.getCode(), InstrumentBean.Status.Leg_3_Settled.getCode(), InstrumentBean.Status.Leg_3_Failed.getCode() };
    //
    ...
    //
    public List<Long> getCurrentUnprocessedList(){
    	return currentUnprocessedList;
    }
    public String[] getCurrentUnprocessedListStr(){
    	return currentUnprocessedListStr;
    }

    //
	public NPCIPaymentSettlor(PaymentFileBean pPaymentFileBean) throws Exception{
        ...
        appEntitiesType = new HashMap<String, NPCIPaymentSettlor.EntityType>();
        ...
        aggregatorPurchaserMapBeanDAO = new GenericDAO<>(AggregatorPurchaserMapBean.class);
	}
	...
	public List<IObligation> getUnProcessedObligations(){
		logger.info("********** PaymentSettlor :: getUnProcessedObligations CALLLED.");
		...
		logger.info("********** PaymentSettlor :: getUnProcessedObligations ENDED.");
		return lObligations;
	}

	public List<IObligation> getAllObligations(){
		logger.info("********** PaymentSettlor :: getAllObligations CALLED.");
		List<IObligation> lObligations = new ArrayList<IObligation>();
        StringBuilder lSql = new StringBuilder(); 
        List<ObligationDetailBean> lObligationDetails = null;
        Map<Long,IObligation> lParentKeys = new HashMap<Long, IObligation>();
        //		
        lSql.append(" SELECT * FROM Obligations, ObligationSplits ");
        lSql.append(" WHERE ");
		...
		logger.info("********** PaymentSettlor :: getAllObligations ENDED.");
		return lObligations;
	}
	...
	public void addObligation(IObligation pObligation){
		...
		if(pObligation !=null){
			...
			NPCIPaymentSettlor.EntityType lEntityType = null;
	...
	public void process(Connection pConnection) throws Exception{
		...
		//
		Map<String, ObligationModificationRequestBean> lCreatedModifiactionRequestHash = getObligationModificationRequests(ObligationModificationRequestBean.Status.Sent.getCode(),paymentFileBean.getId());
		if (lCreatedModifiactionRequestHash!=null && !lCreatedModifiactionRequestHash.isEmpty()) {
			throw new CommonBusinessException("Some modification requests are created. Please approve or reject the requests.");
		}
		Map<String, ObligationModificationRequestBean> lSentModifiactionRequestHash = getObligationModificationRequests(ObligationModificationRequestBean.Status.Created.getCode(),paymentFileBean.getId());
		if (lSentModifiactionRequestHash!=null && !lSentModifiactionRequestHash.isEmpty()) {
			throw new CommonBusinessException("Some modification requests are sent. Please approve or reject the requests.");
		}
		List<IObligation> lUnProcessedList = getUnProcessedObligations();
		List<IObligation> lAllObligations = getAllObligations();
		//
		//
		Map<String, ObligationModificationRequestBean> lModifiactionRequestHash = getObligationModificationRequests(ObligationModificationRequestBean.Status.Approved.getCode(),null);
		//
		...
		//newObligationList is the unprocessed list
		//
		//ObligationWise - Sent Count
		//
		//HashMap<IParentObligation, List<IChildObligation>>
		//Loop through IParent Obligations
		//start a transaction for that parent only
		//loop through child obligaitons and check whether all are not sent and not prcoessed
		//if all are ready for processing then update the childs (as processed) and then update the parent status
		//
		//
		//Key1=ObligationType(L1/L2)   Key2=EntityType(B/S/T)  Key3=UniqueObliKey(ObId when No Split/ "ObId-PartNo" when split
		//Leg 1 Obligations
		//
		//update all the childs to processed if no obligation is sent
				
		//update parent obligations if all splits are not in SENT
		updateParentObligaitons();
		updateParentObligaitonsForReadySplits(lUnProcessedList);
	    ...
	    sendL1DetailsToClient(CostBearer.Seller);
	    sendL1DetailsToClient(CostBearer.Buyer);
	    sendL2DetailsToClient();
	    //
	    int lCount = 0;
	    CashInvoicePaymentAdviceGenerator lCashInvoicePaymentAdviceGenerator = new CashInvoicePaymentAdviceGenerator();
	    try {
		    lCount = lCashInvoicePaymentAdviceGenerator.cashInvoicePayAdvice(pConnection, paymentFileBean.getDate());
	    }catch(Exception lEx) {
	    	logger.info("Error in process while generating Cash Invoice payment advice. Error : "+lEx.getMessage());
	    }
	    logger.info("Payment Advice generate count : " + lCount);
	    //
	}
	private void updateTunoverFinL1() throws Exception{
		logger.info("********** PaymentSettlor :: updateTunoverFinL1 CALLED.");
		StringBuilder lSql = new StringBuilder();
		DBHelper lDBHelper = DBHelper.getInstance();
		...
		lSql.append(" USING ( ");
		lSql.append(" SELECT FUFINANCIER Financier ");
		...
		lSql.append(" ON ( ");
		lSql.append(" MTCODE = FUFINANCIER  ");
		...
		lSql.append(" GROUP BY FUFINANCIER ");
		lSql.append(" ) CalAmt ");
		lSql.append(" ON  ");
		lSql.append(" ( ");
		lSql.append(" MTCode = Financier  ");
		lSql.append(" AND MTFINYEARSTARTDATE = ").append(lDBHelper.formatDate(lFYStartDate));
		lSql.append(" AND MTFINYEARENDDATE = ").append(lDBHelper.formatDate(lFYEndDate));
		lSql.append(" ) ");
		...
		logger.info("********** PaymentSettlor :: updateTunoverFinL1 ENDED.");
	}
	
	private void updateTunoverL1() throws Exception{
		logger.info("********** PaymentSettlor :: updateTunoverL1 CALLED.");
		StringBuilder lSql = new StringBuilder();
		DBHelper lDBHelper = DBHelper.getInstance();
		Date lFYEndDate = null , lFYStartDate = null;
		Date[] lTmpDates = TredsHelper.getInstance().getFinYearDates(TredsHelper.getInstance().getBusinessDate());
		...
		logger.info("********** PaymentSettlor :: updateTunoverL1 ENDED.");
		///test_TURNOVER();
	}
	...
	private void updateFactoringUnitStatus() throws Exception{
		...
		DBHelper lDBHelper = DBHelper.getInstance();
		int lUpdateCount = 0;
		logger.info("********** PaymentSettlor :: Update Factoring Unit Status. CALLED");
		...
		logger.info("********** PaymentSettlor :: Update Factoring Unit Status. ENDED");
	}
	...
	private void updateInstrumentStatus() throws Exception{
		...
		DBHelper lDBHelper = DBHelper.getInstance();
		int lUpdateCount = 0;
		logger.info("********** PaymentSettlor ::  Update Instrument Status. CALLLED.");
		for(int lPtr=0; lPtr < IDX_RESULT_SIZE; lPtr++){
			lSql.delete(0, lSql.length());
		...
		logger.info("********** PaymentSettlor ::  Update Instrument Status. ENDED.");
	}
	...
	public void generateNoticeOfAssignment(Connection pConnection, Long pPaymentFileId, Date pLeg1SettledDate) {
		...
		DBHelper lDBHelper = DBHelper.getInstance();
		logger.info("********** PaymentSettlor :: NOA CALLED.");
		...
		try{
			...
			if(pPaymentFileId == null || pPaymentFileId.longValue() <= 0){
				logger.info("********** PaymentSettlor :: Payment file Id required.");
				return;
			}
			logger.info("********** PaymentSettlor :: Payment file Id : " + pPaymentFileId);
		...
		logger.info("********** PaymentSettlor :: NOA ENDED.");
	}
...
//	}
	...
	private NPCIPaymentSettlor.EntityType getEntityType(String pEntityCode){
		NPCIPaymentSettlor.EntityType lType = null;
			...
			try {
				...
				if(lAppEntityBean!=null){
					if(lAppEntityBean.isFinancier())
						lType = NPCIPaymentSettlor.EntityType.Financier;
					else if(lAppEntityBean.isPurchaser())
						lType = NPCIPaymentSettlor.EntityType.Buyer;
					else if(lAppEntityBean.isSupplier())
						lType = NPCIPaymentSettlor.EntityType.Seller;
					else if(lAppEntityBean.isPlatform())
						lType = NPCIPaymentSettlor.EntityType.Treds;
	...
	private void updatePaymentFileStatus(){
		logger.info("********** PaymentSettlor :: updatePaymentFileStatus CALLED.");
		StringBuilder lSql = new StringBuilder();
		if(PayfileType.Interim_Debit_File.equals(paymentFileBean.getPayfileType()) ||
				PayfileType.Interim_Credit_File.equals(paymentFileBean.getPayfileType())){
			lSql = new StringBuilder();
			lSql.append(" SELECT ");
			lSql.append("  Sum(CASE WHEN OBLIGATIONSPLITS.OBSSettlorProcessed IS NULL AND OBSSTATUS!='SNT' THEN 1 ELSE 0 END) TotalUnProcessed ");
			lSql.append(" FROM OBLIGATIONSPLITS, Obligations ");
			lSql.append(" WHERE OBSObId = OBId ");
			lSql.append(" AND OBSRecordVersion > 0 AND OBRecordVersion > 0 ");
			lSql.append(" AND OBPFId = ").append(paymentFileBean.getId());
			lSql.append(" AND OBSSTATUS!='SNT' AND OBSSettlorProcessed IS NULL ");
			...
			if (lTotalUnProcessed!=null && lTotalUnProcessed.equals(new Long(0))) {
				paymentFileBean.setStatus(PaymentFileBean.Status.Interim_File_Processed);
				try {
					paymentFileDAO.update(connection, paymentFileBean);
				} catch (SQLException e) {
					logger.info("Error in updatePaymentFileStatus D FILE OR C FILE : "+ e.getMessage());
					e.printStackTrace();
				}
			}
		}else{
		...
		logger.info("********** PaymentSettlor :: updatePaymentFileStatus ENDED.");
	...
	//TOCHECK : 
	private Map<String, ObligationModificationRequestBean> getObligationModificationRequests(String pStatus,Long pPayFileId) {
		StringBuilder lSql = new StringBuilder();
		DBHelper lDbHelper = DBHelper.getInstance();
		logger.info("********** PaymentSettlor :: getApprovedObligationModificationRequests CALLED.");
		lSql.append(" SELECT * FROM ( ");
		if (pPayFileId!=null) {
			lSql.append(" SELECT * from OBLIGATIONSMODIREQUESTS , OBLIGATIONSMODIDETAILS, OBLIGATIONS WHERE OMRID=OMDOMRID AND OBFUID=OMRFUID AND OBPFID = ").append(pPayFileId);
		}else {
			lSql.append(" SELECT * from OBLIGATIONSMODIREQUESTS , OBLIGATIONSMODIDETAILS WHERE OMRID=OMDOMRID ");
		}
		lSql.append(" ) ");
		...
		lSql.append(" AND OMRSTATUS = ").append(lDbHelper.formatString(pStatus));
		...
		logger.info("********** PaymentSettlor :: getApprovedObligationModificationRequests ENDED.");
		return null;
	...
	private void updateSplitsFromModRequests(List<IObligation> pUnProcessedList, Map<String, ObligationModificationRequestBean> pModifiactionRequestHash) {
		String lKey = null;	
		logger.info("********** PaymentSettlor :: updateSplitsFromModRequests CALLED.");
		ObligationModificationRequestBean lRequestBean = null;
		modifiedReadyObliList = new ArrayList<Long>();
			...
			if (pModifiactionRequestHash.containsKey(lKey)){
				...
				for(ObligationModificationDetailBean lDetailBean : lRequestBean.getObliModDetailsList()){
					if(lDetailBean.getObId().equals(lBean.getId()) && lRequestBean.getPartNumber().equals(lBean.getPartNumber())){
						...
						if (Status.Ready.equals(lBean.getStatus())
								|| Status.Created.equals(lBean.getStatus())){
							logger.info("new Status Ready");
							lBean.setPfId(null);
							//keep the paydetails same 
							lBean.setPaymentRefNo(null);
							lBean.setFileSeqNo(null);
							if (!modifiedReadyObliList.contains(lBean.getId())){
								modifiedReadyObliList.add(lBean.getId());
							}
						}
		...
		if (!modifiedReadyObliList.isEmpty()){
			modifiedReadyObliListStr = TredsHelper.getInstance().getCSVIdsListForInQuery(modifiedReadyObliList);
		}
		logger.info("********** PaymentSettlor :: updateSplitsFromModRequests ENDED.");
    }
	
	private void markUnprocessedSplitsAsProcessed(List<IObligation>lUnProcessedList){
		logger.info("********** PaymentSettlor :: markUnprocessedSplitsAsProcessed ENDED.");
		for(IObligation lBean : lUnProcessedList){
			if(lBean instanceof ObligationSplitsBean){
				ObligationSplitsBean lOBSBean = (ObligationSplitsBean) lBean;
				if (!Status.Ready.equals(lOBSBean.getStatus())){
	...
	public List<Map<String,Object>>  getObligationsForFinalStatusUpdate(){
		logger.info("********** PaymentSettlor :: getObligationsForFinalStatusUpdate CALLED.");
        StringBuilder lSql = new StringBuilder(); 
        DBHelper lDBHelper = DBHelper.getInstance();
        Statement lStatement = null;
        ResultSet lResultSet = null;
...
//      lSql.append(" AND OBPFID = ").append(paymentFileBean.getId());
		...
		logger.info("********** PaymentSettlor :: getObligationsForFinalStatusUpdate ENDED.");
        return lResultList;
	}

	private void updateParentObligaitonsForReadySplits(List<IObligation> pUnprocessedObli) {
		StringBuffer lSql = new StringBuffer();
		logger.info("********** PaymentSettlor :: updateParentObligaitonsForReadySplits CALLED.");
		if(modifiedReadyObliListStr == null || modifiedReadyObliListStr.length == 0) {
			return ;
		}
		if(modifiedReadyObliListStr.length > 0 && StringUtils.isEmpty(modifiedReadyObliListStr[0])){
			return;
		}
		lSql.append(" Select count(*) TotalCount");
		lSql.append(" , Sum(CASE WHEN OBSSTATUS in ('RDY','CRT') THEN 1 ELSE 0 END ) TotalReadyCreated ");
		lSql.append(" FROM OBLIGATIONSPLITS ");
		lSql.append(" LEFT OUTER JOIN Obligations on (OBSOBID = OBID) ");
		lSql.append(" LEFT OUTER JOIN FactoringUnits on (OBFUID = FUID) ");
		lSql.append(" WHERE OBSRECORDVERSION>0 AND OBRECORDVERSION>0 AND FURECORDVERSION>0 ");
		lSql.append(" AND ").append(TredsHelper.getInstance().getInQuery("OBId",modifiedReadyObliListStr)).append("  ");
		lSql.append(" GROUP BY OBFUID,OBSSTATUS,OBDATE ");
		logger.info("Fetching Ready Count" + lSql.toString());
		//
		Long lTotalReadyCreated = null;
		Long lTotalCount = null;
		try (Statement lStatement = connection.createStatement();
				ResultSet lResultSet = lStatement.executeQuery(lSql.toString());){
		    while (lResultSet.next()){
		    	lTotalReadyCreated = lResultSet.getLong("TotalReadyCreated");
		    	lTotalCount = lResultSet.getLong("TotalCount");
		    }
		}catch(Exception lException){
			
		}
		//
		Map <Long,Date> lDateHash = new HashMap<Long,Date>();
		if (pUnprocessedObli!=null && pUnprocessedObli.size() > 0){
			for (IObligation lBean:pUnprocessedObli){
				if (Status.Ready.equals(lBean.getStatus()) && !lDateHash.containsKey(lBean.getId())){
					lDateHash.put(lBean.getId(), lBean.getDate());
				}
			}
			//
			if(lTotalReadyCreated!=null && lTotalCount!=null && lTotalReadyCreated.equals(lTotalCount)){
				lSql = new StringBuffer();
				lSql.append(" SELECT * FROM OBLIGATIONS,ObligationSplits ");
				lSql.append(" WHERE ").append(TredsHelper.getInstance().getInQuery("OBId",modifiedReadyObliListStr)).append("  ");
				lSql.append(" AND OBSOBID=OBID order by OBID ");
				try {
					List<ObligationDetailBean> lObligations = obligationDetailDAO.findListFromSql(connection, lSql.toString(), -1);
					HashMap<Long,ObligationBean> lParentsHash = new HashMap<>(); 
					HashMap<Long,BigDecimal> lAmtHash = new HashMap<>(); 
					ObligationBean lObligBean = null;
					ObligationSplitsBean lSplitBean = null;
					BigDecimal lAmt = BigDecimal.ZERO;
					for(ObligationDetailBean lBean : lObligations){
						lObligBean = lBean.getObligationBean();
						lSplitBean = lBean.getObligationSplitsBean();
						if (!lParentsHash.containsKey(lObligBean.getId())) {
							lParentsHash.put(lObligBean.getId(), lObligBean);
						}
						if (!lAmtHash.containsKey(lSplitBean.getId())) {
							lAmtHash.put(lSplitBean.getId(), BigDecimal.ZERO);
						}
						lAmt = lAmtHash.get(lSplitBean.getId()).add(lSplitBean.getAmount());
						lAmtHash.put(lSplitBean.getId(), lAmt);
					}
					for(ObligationBean lBean : lParentsHash.values()){
						//To:Do : add record updater 
						obligationDAO.insertAudit(connection, lBean, AuditAction.Update, new Long(0));
						lBean.setDate(lDateHash.get(lBean.getId()));
						lBean.setPfId(null);
						lBean.setStatus(Status.Ready);
						lBean.setPayDetail1(null);
						lBean.setPayDetail2(null);
						lBean.setPayDetail3(null);
						lBean.setPayDetail4(null);
						lBean.setPaymentRefNo(null);
						lBean.setAmount(lAmtHash.get(lBean.getId()));
						obligationDAO.update(connection, lBean);
					}
				}catch(Exception lException){
					logger.info(lException.getMessage());
				}
			}
		}
		logger.info("********** PaymentSettlor :: updateParentObligaitonsForReadySplits ENDED.");
	}
	...
	private Map<Long, Object[]> getL1Obligations(CostBearer pCostBearer) throws Exception{
        StringBuilder lSql = new StringBuilder(); 
        List<ObligationDetailBean> lObligationDetails = null;
        DBHelper lDbHelper = DBHelper.getInstance();
        List<String> lPurchaserList = new ArrayList<>();
        List<String> lAdapterEntityList = ClientAdapterManager.getInstance().getClientEntityList();
        if (!lAdapterEntityList.isEmpty()) {
        	 for  (String lEntityCode : lAdapterEntityList) {
             	AppEntityBean lEntityBean = TredsHelper.getInstance().getAppEntityBean(lEntityCode);
             	if (lEntityBean!=null){
             	if (lEntityBean.isPurchaserAggregator()) {
             		AggregatorPurchaserMapBean lBean = new AggregatorPurchaserMapBean();
             		lBean.setAggregator(lEntityCode);
             		List<AggregatorPurchaserMapBean> lAggPurList = aggregatorPurchaserMapBeanDAO.findList(connection, lBean);
             		if (!lAggPurList.isEmpty()) {
	                   	 for  (AggregatorPurchaserMapBean lAggBean : lAggPurList) {
	                   		if (!lPurchaserList.contains(lAggBean.getPurchaser())) {
	                   			lPurchaserList.add(lAggBean.getPurchaser());
	                   		}
	                   	 }
             		}
             	}else {
             		lPurchaserList.add(lEntityCode);
             	}
             	}else {
             		logger.info("error entity not found : "+lEntityCode);
             	}
             }
        }
        ...
        lSql.append(" LEFT OUTER JOIN FACTORINGUNITS ON ( OBFUID = FUID  AND  FUPURCHASER IN ( ").append(TredsHelper.getInstance().getCSVStringForInQuery(lPurchaserList)).append( " ) ) " );
        lSql.append(" WHERE OBSOBID = OBID AND OBPFID = ").append(paymentFileBean.getId());
        if (CostBearer.Seller.equals(pCostBearer)) {
        	lSql.append(" and OBTXNENTITY=FUSUPPLIER" );
        }else if(CostBearer.Buyer.equals(pCostBearer)){
        	lSql.append(" and OBTXNENTITY=FUPURCHASER" );
        }
        lSql.append(" AND ");
	...
	private void sendL1DetailsToClient(CostBearer pCostBearer){
		Map<Long, Object[]> lObligationMap , lFactoredMap = null;
		List<Object[]> lList = new ArrayList<Object[]>();
		try {
			lObligationMap = getL1Obligations(pCostBearer);
			if(lObligationMap!=null && lObligationMap.size() > 0) {
			lFactoredMap = TredsHelper.getInstance().getFactoredBeans(connection,lObligationMap);
				if(lFactoredMap!=null && lFactoredMap.size() > 0) {
			ObligationBean lOBBean = null;
			Object[] lData = null;
			for (Object[] lTmp : lObligationMap.values()){
				lData = new Object[4];
						if(lTmp!=null && lTmp.length > 1) {
				lOBBean = (ObligationBean) lTmp[0];
							if(lOBBean!=null && lOBBean.getFuId()!=null) {
				Object[] lTmpFactored = lFactoredMap.get(lOBBean.getFuId());
								if(lTmpFactored!=null && lTmpFactored.length > 1) {
				...
				lList.add(lData);
									...
									logger.info("sendL1DetailsToClient : FactoredBean null or empty.");
								}
							}else {
								logger.info("sendL1DetailsToClient : ObligationBean null or empty.");
							}
						}else {
							logger.info("sendL1DetailsToClient : ObligationMapData null or empty.");
						}
					}
				}else {
					logger.info("sendL1DetailsToClient : FactoredMap null or empty.");
				}
			}else {
				logger.info("sendL1DetailsToClient : No supplier obligation map found.");
			}
		} catch (Exception e) {
			logger.error("Error in sendL1DetailsToClient : ",e);
		}
