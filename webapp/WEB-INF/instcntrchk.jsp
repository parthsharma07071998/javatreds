...
<!-- frmSearch -->
			...
			<div>
				<div class="cloudTabs">
	                 <ul class="cloudtabs nav nav-tabs">
						 ...
						 <li style="display:none"><a href="#tab2" data-toggle="tab"><span id="spnTab2">Counter</span> Pending <span id="badge2" class="badge badge-primary"></span></a></li>
						 <li><a href="#tab3" data-toggle="tab" style='display:none'>Ready For Auction <span id="badge3" class="badge badge-primary"></span></a></li>
						 <li style="display:none"><a href="#tab4" data-toggle="tab">Rejected <span id="badge4" class="badge badge-primary"></span></a></li>
						 ...
						 <li><a href="#tab11" data-toggle="tab">Checker Returned <span id="badge11" class="badge badge-primary"></span></a></li>
				 	</ul>
				</div>
				<div class="filter-block clearfix">
					<div class="">
						<span class="right_links">
							<a class="secure btn-group0 btn-group9" href="javascript:;" data-seckey="instcntrchk-approve" id=btnViewApprove><span class="glyphicon glyphicon-ok"></span> Approve</a>
							<a class="secure btn-group0 btn-group9" href="javascript:;" data-seckey="instcntrchk-return" id=btnReturn><span class="glyphicon glyphicon-remove"></span> Return</a>
							<a class="secure " href="javascript:;" data-seckey="instcntrchk-view" id=btnClubbedInstruments><span class="glyphicon glyphicon-eye"></span> View Group Details</a>
	...
	<script type="text/javascript">
		...
		var MAIN_RESOURCE = "instcntrchk";
		//
		$(document).ready(function() {
			MAIN_RESOURCE = "instcntrchk";
			...
			$('#btnApprove').on('click', function() {
				updateStatus('<%=InstrumentBean.Status.Counter_Approved.getCode()%>',$('#btnApprove'));
			});
			$('#btnReturn').on('click', function() {
				updateStatus('<%=InstrumentBean.Status.Counter_Checker_Return.getCode()%>',$('#btnReturn'));
			});
				...
				var lRows=crudInstrument.getSelectedRows();
		 		if(lRows.length!=1){
		 			if(lRows.length<1){
		 				...
		 				updateStatus('<%=InstrumentBean.Status.Counter_Approved.getCode().toString()%>')
		 			}
		 		}else{
		 			...
		 			if (lData == '<%=InstrumentBean.Status.Counter_Checker_Pending.toString()%>'){
		 				showRemote('instview?couchkapp=Y&id='+lDataId, 'modal-xl', true);
					}else{
			...
			var lRows=crudInstrument.getSelectedRows();
	 		if(lRows.length<1){
	 			...
	 			prompt("Please enter suitable reason/remarks","Reason",function(pReason){
					...
					var lStatus = pStatus;
					$.ajax( {
			            url: "instcntrchk/status/"+pStatus.toLowerCase(),
			            type: "POST",
